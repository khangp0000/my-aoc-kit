...
fn solve_once_for_both(shared: &mut SharedData) -> &CommonResult {
    shared.common_result.get_or_insert_with(|| {
        let (_, zero_counts, pass_zero_counts) = shared.parsed_input.iter()
            .fold((50i16, 0u16, 0_u16), |(mut dail_value, mut zero_counts, mut pass_zero_counts), rotate_val| {
                let old_dail_value = dail_value;
                dail_value += rotate_val;
                if dail_value <= 0 && old_dail_value != 0 {
                    pass_zero_counts += 1;
                }
                pass_zero_counts += (dail_value / 100).unsigned_abs();
                dail_value %= 100;
                if dail_value < 0 {
                    dail_value += 100;
                }
                ...
                (dail_value, zero_counts, pass_zero_counts)
            });
